                ...
                <div class="modal-body">
                    ...
                    <div id="formPembelian">
                        ...
                        <div class="input-group mb-3">
                            ...
                            <?php
                                    include 'koneksi.php';
                                    $sql = "select namakry from karyawan";
                            ...
                            <?php
                                    include 'koneksi.php';
                                    $sql = "select namaperusahaan from supplier";
                                    $hasil =  mysqli_query($conn,$sql);
                            ?>
                                <select class="custom-select" id="supplier">
                                <?php                                        
                                    if(mysqli_num_rows($hasil)>0)
                                    {
                                        while($isi=mysqli_fetch_assoc($hasil))
                                        {
                                            $idkodebr=$isi["kodesupp"];
                                            echo '<option>';
...
//------------REMOVE DARI TABEL PENJUALAN PEMBELIAN
...
//------------SAVE TABEL MASTER
      $("#savemasterdetail").click(function () {
        var kodepemesanan1 = <?php echo date('YmdHis');  ?>;
        var tgl1 = $("#tgl").val();
        var karyawan1 = $("#karyawan").val();
        var supplier1 = $("#supplier").val();
        var keterangan1 = $("#keterangan").val();
        var total1 = $("#total").text();
        console.log(kodepemesanan1);
        console.log(tgl1);
        ...
        $("#tbody tr").each(function () {
          var currentRow = $(this);
          var kodebr1=currentRow.find("td:eq(0)").text();
          var hargajual1=currentRow.find("td:eq(3)").text();
          var jumlah1=currentRow.find("td:eq(4)").text();

          console.log(kodepemesanan1);
          console.log(hargajual1);
          console.log(jumlah1);

          $.post("simpandetailpemesanan.php",{
            kode1:kodepemesanan1,
            kodebr:kodebr1,
        ...
        $.post("simpanmasterpemesanan.php",{
            kodepemesanan:kodepemesanan1,
            tanggal:tgl1,
            karyawan:karyawan1,
            supplier:supplier1,
